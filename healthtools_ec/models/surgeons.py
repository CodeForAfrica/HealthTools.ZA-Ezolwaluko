...
class RegisterForm(Form):
    ...
    name = StringField(
        "Initiate Name", [validators.Length(max=50), validators.DataRequired()]
    )
    phone_number = StringField(
        "Phone Number", [validators.Length(min=10, max=10), validators.DataRequired()]
    )
    area = StringField(
        "Area where you work in",
        [validators.Length(max=255), validators.DataRequired()],
    )
...
class FindSurgeonForm(Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.location.choices = [
            [str(i), row.area] for i, row in enumerate(query.all())
        ]
